...
//    vec3(Real const&x, Real const&y, Real const&z): data_(std::valarray{x, y, z}){}
         ...
         */
        void add(vec3<Real> const& v) { data_ +=v.data_; }
         ...
         */
        void subtract(vec3<Real> const& v) { data_ -= v.data_; }
         ...
         */
        Real dot(vec3<Real> const& v) const { return (data_ * v.data_).sum(); }
         ...
         * @param rhs right hand side oif the `+=` operator
         */
        friend void operator+=(vec3<Real>& lhs, vec3<Real> const& rhs) { lhs.add(rhs); }
         ...
         * @param rhs right hand side oif the `-=` operator
         */
        friend void operator-=(vec3<Real>& lhs, vec3<Real> const& rhs) { lhs.subtract(rhs); }
