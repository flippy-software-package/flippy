...
TEST_CASE("Benchmark vec3", "[!benchmark]")
{
    ...
    SECTION("dot") {
#define dot_bench meter.measure([&] { return v0.dot(v1); });
        BASE_EXPERIMENTAL_COMPARE(dot_bench)
    }

    SECTION("norm") {
#define norm_bench meter.measure([&] { return v0.norm(); });
        BASE_EXPERIMENTAL_COMPARE(norm_bench)
    }

    SECTION("norm_square"){
#define norm_square_bench meter.measure([&] { return v0.norm_square(); });
        BASE_EXPERIMENTAL_COMPARE(norm_square_bench)
    }


    SECTION("two vector subtract"){
#define subtraction_bench meter.measure([&]{return v0-v1;});
        BASE_EXPERIMENTAL_COMPARE(subtraction_bench)
    };

    SECTION("vector minus"){
#define vector_minus_bench meter.measure([&]{return -v1;});
        BASE_EXPERIMENTAL_COMPARE(vector_minus_bench)
    };

    SECTION("two vector add"){
#define add_bench meter.measure([&]{return v0+v1;});
        BASE_EXPERIMENTAL_COMPARE(add_bench)
    };

    SECTION("multiply left"){
#define multiply_left_bench auto r = rand[4]; meter.measure([&]{return r*v1;});
        BASE_EXPERIMENTAL_COMPARE(multiply_left_bench)

        };

    SECTION("multiply right"){
#define multiply_right_bench auto r = rand[4]; meter.measure([&]{return v1*r;});
        BASE_EXPERIMENTAL_COMPARE(multiply_left_bench)

    };

    SECTION("complex expression"){
#define complex_expression_bench auto r = rand[4]; meter.measure([&]{return (v1*r + v0*v1.dot(v0)).cross(v0)-v0;});
