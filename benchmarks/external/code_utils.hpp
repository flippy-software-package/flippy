...
template <typename C>
concept Container = Beginable<C> && Endable<C> && NeqableBeginAndEnd<C> && BeginIncrementable<C> && BeginDerefable<C> &&
                    !BeginDerefToVoid<C> && BeginAndEndCopyConstructibleAndDestructible<C>;
...
template <typename T>
concept Printable = StdOutStreamable<T> || Container<T>;

template <char sep, char end> static void print();
template <char sep, char end> static void print(const Container auto &output);
template <char sep, char end> static void print(const Printable auto &first, const Printable auto &...rest);
...
template <char sep = ' ', char end = '\n'> [[maybe_unused]] void print(const Container auto &output) {
    print<sep, ' '>('{');
