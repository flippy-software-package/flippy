...
template <typename Index> std::vector<Index> rotate_left(std::vector<Index> v) {
    std::rotate(v.begin(), v.begin() + 1, v.end());
    return v;
}

const fp::Json ICOSA_DATA =
    R"({
...
void radius_scaling_test(const Triangulation &triangulation, Real r_init) {
...
TEST_CASE("Proper scaling with initial Radius for Triangulation Instantiator") {
    ...
    SECTION("loading data instantiation") {
        auto triangulation = Triangulation::load_sphere_from_json(ICOSA_DATA, 0);
    ...
    SECTION("spherical triangulator data instantiation") {
        auto triangulation = Triangulation::make_spherical_triangulation(0, r_init, 0);
        radius_scaling_test(triangulation, r_init);
    }
}
...
TEST_CASE("Proper move") {
    Real r_init              = 1_r;
    auto icosa_triangulation = Triangulation::load_sphere_from_json(ICOSA_DATA, 0);
    ...
    SECTION("Just CHECK if move_node reverses things correctly") {
        ...
        CHECK((copy_mc - mc).norm() == Catch::Approx(0).margin(0.001));
        for (Index i = 0; i < icosa_triangulation.size(); ++i) {
            CHECK(copy_icosa_triangulation.nodes_[i].area ==
                  Catch::Approx(icosa_triangulation.nodes_[i].area).margin(0.01));
            ...
            for (std::size_t j = 0; j < copy_icosa_triangulation.nodes_[i].nn_ids.size(); ++j) {
                CHECK(
                    (copy_icosa_triangulation.nodes_[i].nn_distances[j] - icosa_triangulation.nodes_[i].nn_distances[j])
                        .norm() == Catch::Approx(0).margin(0.01));
            }
        }
    }
}
...
TEST_CASE("Proper topology change") {
    ...
    SECTION("flip_bond unit test") {
        auto icosa = Triangulation::load_sphere_from_json(ICOSA_DATA, 0);
    ...
    SECTION("Property check: unflip reverses flip") {
        ...
        auto sphere = Triangulation::make_spherical_triangulation(10, r_init, 0);
        ...
        for (unsigned int repeat = 0; repeat < 10; ++repeat) {
            ...
            CHECK(bfd.flipped == true);
            CHECK(Catch::Approx(global_geometry_ORIGINAL.area).margin(small_number) ==
                  global_geometry_AFTER_FLIP_BACKFLIP.area);
            CHECK(Catch::Approx(global_geometry_ORIGINAL.volume).margin(small_number) ==
                  global_geometry_AFTER_FLIP_BACKFLIP.volume);
            ...
            bool nn_ids_are_directly_equal_or_equal_after_odd_perm =
                (nn_ids == sphere[rand_node_id].nn_ids) || (nn_ids == rotate_left(sphere[rand_node_id].nn_ids));
    ...
    SECTION("CHECK new two common neighbours on STAR_DATA examples") {
        auto      star_triangulation    = Triangulation::load_sphere_from_json(STAR_DATA, 0);
    ...
    //    SECTION("property test: common_neighbours and two_common_neighbours should give same results apart from
    //    ordering") {
...
TEST_CASE("emplace_before unit test") {
    ...
    SECTION("emplace_before at the beginning") {
        auto trg = Triangulation::load_sphere_from_json(ICOSA_DATA, 0);
...
TEST_CASE("unittest private member functions") {
    SECTION("unittest all_nn_distance_vectors") {
        ...
        std::vector<vec3<Real>> all_nn_d =
            star_triangulation[8].nn_distances; // star_triangulation.all_nn_distance_vectors(8);
    ...
    SECTION("order_nn_ids: simple test on icosa data") {
        auto icosa_triangulation_load = Triangulation::load_sphere_from_json(ICOSA_DATA, 0);
