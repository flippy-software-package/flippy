...
TEST_CASE("Proper topology change")
{
    ...
    SECTION("flip_bond unit test"){
        ...
        CHECK(icosa[2].nn_ids==std::vector<unsigned long>{3,0,7,8});
        CHECK(icosa[0].nn_ids==std::vector<unsigned long>{3,4,5,1,7,2});
        CHECK(icosa[7].nn_ids==std::vector<unsigned long>{2,0,1,6,11,8});
    }
    ...
    SECTION("Property check: unflip reverses flip"){
        ...
        Triangulation<double, idx, SPHERICAL_TRIANGULATION> sphere(10, r_init, 0);
        std::random_device rd;
        std::mt19937_64 rng(rd());
        std::uniform_int_distribution<idx> rid(0, sphere.size()-1);
        for(unsigned int repeat=0; repeat<10; ++repeat) {
            idx rand_node_id = rid(rng);
            auto nn_ids = sphere[rand_node_id].nn_ids;
            std::uniform_int_distribution<idx> r_nn_id(0, nn_ids.size()-1);
            idx rand_nn_id = nn_ids[r_nn_id(rng)];
    ...
    SECTION("CHECK two common neighbours and normal common neighbours on icosa examples") {
        ...
        std::array<unsigned long, 2> two_cnns = icosa_triangulation.two_common_neighbours(11, 1);
        std::vector<unsigned long> cnns = icosa_triangulation.common_neighbours(11, 1);
        std::sort(two_cnns.begin(), two_cnns.end());
        std::sort(cnns.begin(), cnns.end());
        CHECK(two_cnns==std::array<unsigned long, 2>{6, 7});
        CHECK(cnns==std::vector<unsigned long>{6, 7});

        two_cnns = icosa_triangulation.two_common_neighbours(5, 7);
        cnns = icosa_triangulation.common_neighbours(5, 7);
        std::sort(two_cnns.begin(), two_cnns.end());
        std::sort(cnns.begin(), cnns.end());
        CHECK(two_cnns==std::array<unsigned long, 2>{1, 6});
        CHECK(cnns==std::vector<unsigned long>{1, 6});
    }

    SECTION("CHECK new two common neighbours on STAR_DATA examples") {
        Triangulation<double, unsigned long, SPHERICAL_TRIANGULATION> star_triangulation(STAR_DATA, 0);
        Neighbors<unsigned long> cnns = star_triangulation.previous_and_next_neighbour_global_ids(2l, 3l);
        Neighbors<unsigned long> cnns_other_way_around = star_triangulation.previous_and_next_neighbour_global_ids(3l, 2l);
    ...
    SECTION("property test: common_neighbours and two_common_neighbours should give same results apart from ordering") {
        ...
        Triangulation<double, unsigned long, SPHERICAL_TRIANGULATION> sphere(nodes, 0);
        std::array<unsigned long, 2> m1m1{VERY_LARGE_NUMBER_, VERY_LARGE_NUMBER_};
        for (unsigned int i = 0; i<sphere.nodes().size() - 1; ++i) {
            for (unsigned int j = i + 1; j<sphere.nodes().size(); ++j) {
                std::vector<unsigned long> cnns = sphere.common_neighbours(i, j);
                std::array<unsigned long, 2> cnns2_arr = sphere.two_common_neighbours(i, j);
                std::vector<unsigned long> cnns2{cnns2_arr[0], cnns2_arr[1]};
                std::sort(cnns.begin(), cnns.end());
...
TEST_CASE("emplace_before unit test"){
    ...
    SECTION("emplace_before at the beginning"){
        Triangulation<float, unsigned long long, SPHERICAL_TRIANGULATION> trg(ICOSA_DATA, 0);
        trg.emplace_before(0, 4, 11);
        CHECK(trg.nodes().nn_ids(0)==std::vector<unsigned long long>{11, 4, 2, 3, 1, 5});
    }

    SECTION("emplace_before in the middle"){
        Triangulation<float, unsigned long long, SPHERICAL_TRIANGULATION> trg(ICOSA_DATA, 0);
        trg.emplace_before(0, 2, 11);
        CHECK(trg.nodes().nn_ids(0)==std::vector<unsigned long long>{4, 11, 2, 3, 1, 5});
    }

    SECTION("emplace_before before the end"){
        Triangulation<float, unsigned long long, SPHERICAL_TRIANGULATION> trg(ICOSA_DATA, 0);
        trg.emplace_before(0, 5, 11);
        CHECK(trg.nodes().nn_ids(0)==std::vector<unsigned long long>{4, 2, 3, 1, 11, 5});
    }

    SECTION("emplace_before nn not found"){
        Triangulation<float, unsigned long long, SPHERICAL_TRIANGULATION> trg(ICOSA_DATA, 0);
        trg.emplace_before(0, 10, 11);
        CHECK(trg.nodes().nn_ids(0)==std::vector<unsigned long long>{4, 2, 3, 1, 5});
    }
...
TEST_CASE("unittest private member functions")
{
    ...
    SECTION("order_nn_ids: simple test on icosa data") {
        Triangulation<double, unsigned long long, SPHERICAL_TRIANGULATION> icosa_triangulation_load(ICOSA_DATA, 0);
        CHECK(icosa_triangulation_load.order_nn_ids(0)==std::vector<unsigned long long>{3, 4, 5, 1, 2});
    }
    ...
    SECTION("orient_surface_of_a_sphere: simple test on icosa data") {
        ...
        CHECK(icosa_triangulation_load[0].nn_ids==std::vector<unsigned long long>{3, 4, 5, 1, 2});
        CHECK(icosa_triangulation_load[1].nn_ids==std::vector<unsigned long long>{6, 7, 2, 0, 5});
        CHECK(icosa_triangulation_load[3].nn_ids==std::vector<unsigned long long>{8, 9, 4, 0, 2});
        CHECK(icosa_triangulation_load[10].nn_ids==std::vector<unsigned long long>{9, 11, 6, 5, 4});
