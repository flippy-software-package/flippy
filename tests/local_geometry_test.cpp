...
TEST_CASE("Icosa geometry check test") {
    Real R           = 20._r;
    auto icosahedron = Triangulation::load_sphere_from_json(ICOSA_DATA(), 0);
    ...
    for (const auto &node : icosahedron.nodes().data) {
...
TEST_CASE("Sphere geometry test") {
    ...
    auto sphere   = Triangulation::load_sphere_from_json(all_data["nodes"], 0);
    ...
    for (const auto &node : sphere.nodes()) {
        lg                += Geometry(node);
...
TEST_CASE("Ellipse geometry test") {
    auto R         = 10_r;
    auto all_data  = json_read("../../tests/init_files/egg_init.json");
    auto x_stretch = 1.4_r;
    auto ellipse   = Triangulation::load_sphere_from_json(all_data["nodes"], 0);
    ...
    vec3<Real> displacement{1200_r, 2329_r, -12.901_r};
    ...
    vec3<Real> displacement2{0_r, 9_r, -1_r};
...
TEST_CASE("Ellipse geometry test for triangulator mesh") {
    auto R         = 2_r;
    auto x_stretch = 1.3_r;
    auto ellipse   = Triangulation::make_spherical_triangulation(15, R, 0);
    ellipse.scale_node_coordinates(x_stretch);

    auto ACCEPTABLE_ERROR = 0.02_r;
...
TEST_CASE("cube geometry test") {
    auto cube             = Triangulation::load_sphere_from_json(CUBE_DATA(), 0);
    Real ACCEPTABLE_ERROR = 0.01_r;
    auto A_square         = Catch::Approx(24_r).margin(ACCEPTABLE_ERROR);
    auto V_square         = Catch::Approx(8_r).margin(ACCEPTABLE_ERROR);
...
TEST_CASE("Brick geometry test") {
    auto brick            = Triangulation::load_sphere_from_json(BRICK_DATA(), 0);
    Real ACCEPTABLE_ERROR = 0.01_r;
...
TEST_CASE("Hyperstretch geometry test") {
    auto brick            = Triangulation::load_sphere_from_json(HYPERBRICK_DATA(), 0);
    Real ACCEPTABLE_ERROR = 0.01_r;
...
TEST_CASE("Hyperqueeze geometry test") {
    auto brick            = Triangulation::load_sphere_from_json(HYPER_SQUEEZED_BRICK_DATA(), 0);
    Real ACCEPTABLE_ERROR = 0.01_r;
