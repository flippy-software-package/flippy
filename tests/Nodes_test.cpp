...
TEST_CASE("getter and setter tests for Nodes"){
    ...
    SECTION("nn_ids"){
        CHECK(icosa_nodes.nn_ids(0) ==std::vector<Index>{4,3,2,1,5});
        CHECK(icosa_nodes.nn_ids(1) ==std::vector<Index>{7,6,2,5,0});
        CHECK(icosa_nodes.nn_ids(2) ==std::vector<Index>{8,7,3,1,0});
        CHECK(icosa_nodes.nn_ids(3) ==std::vector<Index>{9,8,4,2,0});
        CHECK(icosa_nodes.nn_ids(4) ==std::vector<Index>{9,10,3,5,0});
        CHECK(icosa_nodes.nn_ids(5) ==std::vector<Index>{6,10,4,1,0});
        CHECK(icosa_nodes.nn_ids(6) ==std::vector<Index>{11,7,10,1,5});
        CHECK(icosa_nodes.nn_ids(7) ==std::vector<Index>{11,8,6,2,1});
        CHECK(icosa_nodes.nn_ids(8) ==std::vector<Index>{11,9,7,3,2});
        CHECK(icosa_nodes.nn_ids(9) ==std::vector<Index>{11,8,10,4,3});
        CHECK(icosa_nodes.nn_ids(10)==std::vector<Index>{11,9,6,4,5});
        CHECK(icosa_nodes.nn_ids(11)==std::vector<Index>{9,8,7,6,10});
    }
    ...
    SECTION("pos"){
        ...
        CHECK(icosa_nodes.pos(1) ==vec3<Real>{89.44271909999158_r, 0.0, 44.721359549995796_r});
        CHECK(icosa_nodes.pos(2) ==vec3<Real>{27.639320225002102_r, 85.06508083520399_r, 44.721359549995796_r});
        CHECK(icosa_nodes.pos(3) ==vec3<Real>{-72.36067977499789_r, 52.57311121191337_r, 44.7213595499958_r});
        CHECK(icosa_nodes.pos(4) ==vec3<Real>{-72.3606797749979_r, -52.57311121191336_r, 44.7213595499958_r});
        CHECK(icosa_nodes.pos(5) ==vec3<Real>{27.639320225002088_r, -85.065080835204_r, 44.7213595499958_r});
        CHECK(icosa_nodes.pos(6) ==vec3<Real>{72.36067977499789_r, -52.57311121191336_r, -44.72135954999579_r});
        CHECK(icosa_nodes.pos(7) ==vec3<Real>{72.36067977499789_r, 52.57311121191336_r, -44.72135954999579_r});
        CHECK(icosa_nodes.pos(8) ==vec3<Real>{-27.639320225002095_r, 85.06508083520399_r, -44.72135954999579_r});
        CHECK(icosa_nodes.pos(9) ==vec3<Real>{-89.44271909999158_r, 1.0953573965284053e-14_r, -44.72135954999579_r});
        CHECK(icosa_nodes.pos(10)==vec3<Real>{-27.639320225002113_r, -85.06508083520399_r, -44.72135954999579_r});
        CHECK(icosa_nodes.pos(11)==vec3<Real>{1.2246467991473532e-14_r, 0.0,-100.0});
    }
    ...
    SECTION("displ"){
        ...
        CHECK((icosa_nodes.pos(1) -vec3<Real>{89.44271909999158_r,0.0,44.721359549995796_r}).norm()==zero);
        CHECK((icosa_nodes.pos(2) -vec3<Real>{27.639320225002102_r, 85.06508083520399_r, 44.721359549995796_r}).norm() ==zero);
...
//        CHECK((icosa_nodes.pos(11)-vec3<Real>{1.2246467991473532e-14,0.0,-100.0}).norm()==zero);
    ...
    SECTION("set_nn_ids"){
        Nodes icosa_nodes_loc(ICOSA_DATA);
        for(Index i = 0; i<12;++i){
            for (Index j = 0; j<5; ++j) {
        ...
        for(Index i = 0; i<12;++i){
            icosa_nodes_loc.set_nn_ids(i, std::vector<Index>{120,12,11});
        }
        for(unsigned int i = 0; i<12;++i){
            CHECK(icosa_nodes_loc.nn_ids(i)==std::vector<Index>{120,12,11});
        }
    }
}